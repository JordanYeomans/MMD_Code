...
class PrepareData():
    def __init__(self, max_utter, max_len, max_images, start_symbol_index, end_symbol_index, unk_symbol_index,
            ...
            for utterance in utterances:
                ...
                if speaker == "system" and (test_state is None or is_test is False or (
                    ...
                    if self.task_type == "image" and (
                            last_utterance['images'] is None or len(last_utterance['images']) == 0):
                        continue
    ...
    def binarize_corpus(self, dialogue_pkl_file):
        if self.task_type == "text":
            self.binarize_corpora(self.dialogue_context_text_task_text_file, self.dialogue_context_image_task_text_file,
                                  self.dialogue_target_text_task_text_file, self.task_type, dialogue_pkl_file)
        if self.task_type == "image":
            self.binarize_corpora(self.dialogue_context_text_task_image_file,
                                  self.dialogue_context_image_task_image_file,
                                  self.dialogue_target_image_task_image_file, self.task_type, dialogue_pkl_file)

    def binarize_corpora(self, dialogue_file_text, dialogue_file_image, dialogue_target_file, task_type,
                         dialogue_pkl_file):
        binarized_corpus = []
        ...
        num_instances = 0
        with open(dialogue_file_text) as textlines, open(dialogue_file_image) as imagelines, open(
            ...
            for text_context, image_context, target in zip(textlines, imagelines, targetlines):
                ...
                if len(binarized_image_context) != self.max_utter:
                    raise Exception('binarized_image_context should be a list of length max_utter, found length ',
